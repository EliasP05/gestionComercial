                ...
                <tr class=" border-b border-slate-200">
                    ...
                    <td class="flex justify-center items-center space-x-1 py-1 ">
                        <a class=" bg-yellow-400 text-white px-2 rounded-md focus:ring-2"
                            href="{{ route('usuarios.edit', $user) }}">{{ __('Edit') }}</a>
        ...
        <tbody class="text-center">
            @foreach ($usuarios as $user)
                <tr class=" odd:bg-white even:bg-slate-50 ">
                    ...
                    <td class="py-2">{{ $user->usu_apellido ?? '' }}</td>
                    ...
                    <td class="flex justify-center items-center space-x-1 py-1 ">
                        <button class="btn-update"
                            href="{{ route('usuarios.edit', $user) }}">{{ __('Edit') }}</button>
                        <form action="{{ route('products.destroy', $user) }}" method="POST">
                            @csrf @method('DELETE')
                            <button class=" btn-delete">{{ __('Delete') }}</button>
                        </form>
                        {{-- //por que la ruta el prodcuts.destroy?  deberia ser usuarios.destroy --}}
                    </td>
                </tr>
            @endforeach
        </tbody>
