...
<div class="container mx-auto">
    <div class=" max-w-4xl mx-auto mt-5 px-5 py-2 rounded-lg border border-gray-300">
        <h1 class="text-center text-3xl ">{{__('Add')}} Usuario</h1>
        <form action="{{route('usuarios.store')}}" method="POST">
            @csrf
            @include('usuarios.forms')
            <div class="flex justify-end space-x-5 items-center">
                 <a class=" bg-gray-500 text-white px-2 py-1 rounded-md font-semibold focus:ring-2" href="{{route('usuarios')}}">{{__('Go Back')}}</a>
